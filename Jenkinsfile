...
node{
    ...
    sh "git clone ${scheme}://jenkins:OqLLOcDzF60mXOyMczC7rO8rZEnCjBEqO0FfjULnZQ@${host}/a/${ctx.project_name}.git mean/code"
    sh """
       cd mean/code
       git fetch ${scheme}://jenkins:OqLLOcDzF60mXOyMczC7rO8rZEnCjBEqO0FfjULnZQ@${host}/a/${ctx.project_name}.git ${ctx.ref} && git checkout FETCH_HEAD
       cd -
       """
    }
    stage("run analyzer") {
